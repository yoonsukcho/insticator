...
<head>
	...
	$(document).ready(function(){
	    $("#next").click(function(){
		   document.location.href='/next?userId=' + $('#userId').val() + '&siteId=' + $('#siteId').val();
		});
	});
	</script>
		...
		<div class="table-responsive">
			<form class="form-horizontal" method="POST" action="submit-answer">
						...
						<c:forEach var="choice" items="${choices}" varStatus="cnt" >
							<tr>
								...
								<c:if test = "${mode != 'SHOW_ANSWER'}">
									<input type="radio" name="answer" value="${cnt.count}"> 
								</c:if>
								<c:if test = "${choice.correct}">
									<input type="hidden" name="rightAnswer" value="${cnt.count}"> 
								</c:if>
								${choice.content} </td>
							</tr>
						</c:forEach>
				...
				<c:if test="${empty lastUserId}">
  					<input id="userId" name="userId" value="testUser" type="text" class="form-control" placeholder="User ID" aria-label="User ID" aria-describedby="basic-addon1">
  				</c:if> 
				<c:if test="${not empty lastUserId}">
  					<input id="userId" name="userId" value="${lastUserId}" type="text" class="form-control" placeholder="User ID" aria-label="User ID" aria-describedby="basic-addon1">
  				</c:if> 
				</div>
